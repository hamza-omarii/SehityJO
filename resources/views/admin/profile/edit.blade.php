...
@section('content')
    ...
    <form action="{{ route('admin.update.profile', Auth::guard('admin')->user()->id) }}" class="p-5 rounded bg-white wow bounceInDown" method="POST">
        @csrf
        @method("PUT")
        <h3 class="text-center primary-text mb-3 fw-bold">{{ __('dashboard.Edit Account Information') }}</h3>
        ...
        <div class="form-floating mb-4">
            ...
            <label for="floatingInput">{{ __('dashboard.email') }}</label>
            @error('email')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
        ...
        <div class="form-floating mb-3">
            ...
            <label for="old-password">{{ __('dashboard.Old Password') }}</label>
        </div>
        @error('old_password')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
        ...
        <div class="row mb-3">
            ...
            <div class="col-6">
                <div class="form-floating">
                    ...
                    <label for="password_confirmation">{{ __('dashboard.password-confirm') }}</label>
                </div>
